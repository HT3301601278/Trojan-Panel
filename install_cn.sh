...
# 作者: jonssonyan <https://jonssonyan.com>
# 文档: https://trojanpanel.github.io
# 译者: 世间微斯人
# 初始化变量
...
check_sys() {
  ...
  if [[ -z "${release}" ]]; then
    echo_content red "操作系统仅支持 CentOS 7+/Ubuntu 18+/Debian 10+"
    exit 0
  fi
  ...
  if [[ -z "${get_arch}" ]]; then
    echo_content red "处理器架构仅支持 amd64/arm64/arm/s390x"
    exit 0
  fi
...
EOF
    ...
    echo_content skyBlue "---> 您已经安装了 Docker"
  fi
}
...
install_custom_cert() {
  if [[ -z "$(cat "${DOMAIN_FILE}")" ]]; then
    while read -r -p "请输入 .crt 证书的文件路径 (必要): " crt_path; do
      if [[ -z "${crt_path}" ]]; then
        echo_content red "路径不能为空"
      else
        if [[ ! -f "${crt_path}" ]]; then
          echo_content red ".crt 证书的文件路径不存在"
        else
    ...
    while read -r -p "请输入 .key 证书的文件路径 (必要): " key_path; do
      if [[ -z "${key_path}" ]]; then
        echo_content red "路径不能为空"
      else
        if [[ ! -f "${key_path}" ]]; then
          echo_content red ".key 证书的文件路径不存在"
        else
...
EOF
    echo_content red "\n=============================================================="
    echo_content skyBlue "---> 自定义设置证书安装完成"
    echo_content yellow "证书目录: ${CERT_PATH}"
    echo_content red "\n=============================================================="
  fi
}

# Caddy2 https 自定义设置证书配置文件
caddy2_https_config() {
  domain=$1
...
install_caddy2() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-caddy$") ]]; then
    echo_content green "---> 安装 Caddy2+https"

    wget --no-check-certificate -O ${WEB_PATH}html.tar.gz -N ${STATIC_HTML} &&
      tar -zxvf ${WEB_PATH}html.tar.gz -k -C ${WEB_PATH}

    read -r -p "请输入 Caddy2 的端口（默认: 80）: " caddy_port
    [[ -z "${caddy_port}" ]] && caddy_port=80
    read -r -p "请输入 Caddy2 的转发端口（默认: 8863）: " caddy_remote_port
    [[ -z "${caddy_remote_port}" ]] && caddy_remote_port=8863

    echo_content yellow "提示：请确认域名已解析到此机器，否则安装可能失败"
    while read -r -p "请输入您的域名（必要）: " domain; do
      if [[ -z "${domain}" ]]; then
    ...
    read -r -p "请输入您的邮箱地址（可选）: " your_email

    while read -r -p "请选择设置证书的方式？（1/自动申请和续订证书 2/手动设置证书路径 默认: 1）: " ssl_option; do
      if [[ -z ${ssl_option} || ${ssl_option} == 1 ]]; then
        while read -r -p "请选择证书申请方式（1/acme 2/zerossl 默认: 1）: " ssl_module_type; do
          if [[ -z "${ssl_module_type}" || ${ssl_module_type} == 1 ]]; then
            ...
            echo_content red "只能输入 1 和 2 以外的其他字符"
          fi
        done
...
EOF
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-caddy$" -f "status=running") ]]; then
      echo_content red "\n=============================================================="
      echo_content skyBlue "---> Caddy2+https 安装完成"
      echo_content yellow "证书目录: ${CERT_PATH}"
      echo_content red "\n=============================================================="
    else
      echo_content red "---> Caddy2+https 安装失败或运行异常，请尝试修复或卸载重装"
      exit 0
    fi
  else
    echo_content skyBlue "---> 您已安装 Caddy2+https"
  fi
}
...
install_nginx() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-nginx$") ]]; then
    echo_content green "---> 安装 Nginx"

    wget --no-check-certificate -O ${WEB_PATH}html.tar.gz -N ${STATIC_HTML} &&
      tar -zxvf ${WEB_PATH}html.tar.gz -k -C ${WEB_PATH}

    read -r -p "请输入 Nginx 的端口（默认：80）：" nginx_port
    [[ -z "${nginx_port}" ]] && nginx_port=80
    read -r -p "请输入 Nginx 的转发端口（默认：8863）：" nginx_remote_port
    [[ -z "${nginx_remote_port}" ]] && nginx_remote_port=8863

    while read -r -p "请选择是否在 Nginx 中启用 https？（0/关闭 1/启用，默认：1）：" nginx_https; do
      if [[ -z ${nginx_https} || ${nginx_https} == 1 ]]; then
        ...
        echo_content red "只能输入 1 或 2 以外的字符"
      fi
    done
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-nginx$" -f "status=running") ]]; then
      echo_content skyBlue "---> Nginx 安装完成"
    else
      echo_content red "---> Nginx 安装失败或运行异常，请尝试修复或卸载并重新安装"
      exit 0
    fi
  else
    echo_content skyBlue "---> 您已安装 Nginx"
  fi
}
...
install_reverse_proxy() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-caddy$|^trojan-panel-nginx$") ]]; then
    echo_content green "---> 安装反向代理服务器"

    while :; do
      echo_content yellow "1. 安装 Caddy2+https（推荐）"
      echo_content yellow "2. 安装 Nginx"
      echo_content yellow "3. 不安装"
    ...
    echo_content skyBlue "---> 反向代理服务器安装完成"
  fi
}
...
install_mariadb() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-mariadb$") ]]; then
    echo_content green "---> 安装 MariaDB"

    read -r -p "请输入 MariaDB 的端口（默认：9507）：" mariadb_port
    [[ -z "${mariadb_port}" ]] && mariadb_port=9507
    read -r -p "请输入 MariaDB 的用户名（默认：root）：" mariadb_user
    [[ -z "${mariadb_user}" ]] && mariadb_user="root"
    while read -r -p "请输入 MariaDB 的密码（必要）：" mariadb_pas; do
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-mariadb$" -f "status=running") ]]; then
      echo_content skyBlue "---> MariaDB 安装完成"
      echo_content yellow "---> MariaDB root 用户密码（请妥善保管）：${mariadb_pas}"
      if [[ "${mariadb_user}" != "root" ]]; then
        echo_content yellow "---> MariaDB ${mariadb_user} 用户密码（请妥善保管）：${mariadb_pas}"
      fi
    else
      echo_content red "---> MariaDB 安装失败或运行异常，请尝试修复或卸载并重新安装"
      exit 0
    fi
  else
    echo_content skyBlue "---> 您已安装 MariaDB"
  fi
}
...
install_redis() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-redis$") ]]; then
    ...
    while read -r -p "请输入 Redis 的密码（必要）：" redis_pass; do
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-redis$" -f "status=running") ]]; then
      ...
      echo_content yellow "---> Redis 密码（请妥善保管）：${redis_pass}"
    else
      echo_content red "---> Redis 安装失败或运行异常，请尝试修复或卸载并重新安装"
      exit 0
    fi
  else
    echo_content skyBlue "---> 您已安装 Redis"
  fi
}

# Trojan Panel 前端前端 Nginx http 配置文件
ui_http_config() {
...
# Trojan Panel 前端 Nginx https 配置文件
ui_https_config() {
  cat >${UI_NGINX_CONFIG} <<-EOF
server {
...
install_trojan_panel_ui() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-ui$") ]]; then
    echo_content green "---> 安装 Trojan Panel 前端"

    read -r -p "请输入 Trojan Panel 后端的 IP 地址（默认：本地主机）：" trojan_panel_ip
    [[ -z "${trojan_panel_ip}" ]] && trojan_panel_ip="127.0.0.1"
    read -r -p "请输入 Trojan Panel 后端的服务端口（默认：8081）：" trojan_panel_server_port
    [[ -z "${trojan_panel_server_port}" ]] && trojan_panel_server_port=8081

    read -r -p "请输入 Trojan Panel 前端的端口（默认：8888）：" trojan_panel_ui_port
    [[ -z "${trojan_panel_ui_port}" ]] && trojan_panel_ui_port="8888"
    while read -r -p "请选择是否在 Trojan Panel 前端启用 https？（0/关闭 1/启用，默认：1）：" ui_https; do
      if [[ -z ${ui_https} || ${ui_https} == 1 ]]; then
        ...
        echo_content red "只能输入 1 或 2 以外的其他字符"
      fi
    done
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
      echo_content skyBlue "---> Trojan Panel 前端安装完成"

      https_flag=$([[ -z ${ui_https} || ${ui_https} == 1 ]] && echo "https" || echo "http")
      domain_or_ip=$([[ -z ${domain} || "${domain}" == "custom_cert" ]] && echo "ip" || echo "${domain}")

      echo_content red "\n=============================================================="
      echo_content skyBlue "Trojan Panel 前端安装成功"
      echo_content yellow "Web 管理面板地址：${https_flag}://${domain_or_ip}:${trojan_panel_ui_port}"
      echo_content red "\n=============================================================="
    else
      echo_content red "---> Trojan Panel 前端安装失败或运行异常，请尝试修复或卸载并重新安装"
      exit 0
    fi
  else
    echo_content skyBlue "---> 您已安装 Trojan Panel 前端"
  fi
}
...
install_trojan_panel() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    echo_content green "---> 安装 Trojan Panel 后端"

    read -r -p "请输入 Trojan Panel 后端的服务端口（默认：8081）：" trojan_panel_port
    [[ -z "${trojan_panel_port}" ]] && trojan_panel_port=8081

    read -r -p "请输入 MariaDB 的 IP 地址（默认：本地主机）：" mariadb_ip
    ...
    read -r -p "请输入 Redis 的 IP 地址（默认：本地主机）：" redis_host
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel$" -f "status=running") ]]; then
      ...
      echo_content yellow "Redis 密码（请妥善保管）：${redis_pass}"
      echo_content yellow "系统管理员默认用户名：sysadmin 默认密码：123456"
      echo_content_yellow "请登录管理面板及时更改密码"
      echo_content red "\n=============================================================="
    else
      echo_content red "---> Trojan Panel 后端安装失败或运行异常，请尝试修复或卸载并重新安装"
      exit 0
    fi
  else
    echo_content skyBlue "---> 您已安装 Trojan Panel 后端"
  fi
}
...
install_trojan_panel_core() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-core$") ]]; then
    echo_content green "---> 安装 Trojan Panel 核心"

    read -r -p "请输入 Trojan Panel 核心的服务端口（默认：8082）：" trojan_panel_core_port
    ...
    read -r -p "请输入 API 端口（默认：8100）：" grpc_port
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-core$" -f "status=running") ]]; then
      echo_content skyBlue "---> Trojan Panel 核心安装完成"
    else
      echo_content red "---> Trojan Panel 核心安装失败或运行异常，请尝试修复或卸载并重新安装"
      exit 0
    fi
  else
    echo_content skyBlue "---> 您已安装 Trojan Panel 核心"
  fi
}
...
# 更新 Trojan Panel 核心 数据库结构
update_trojan_panel_core_database() {
  echo_content skyBlue "---> 更新 Trojan Panel 核心 数据库结构"

  echo_content skyBlue "---> Trojan Panel 核心 数据库结构更新完成"
}

# 更新 Trojan Panel 前端
update_trojan_panel_ui() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-ui$") ]]; then
    echo_content red "---> 请先安装 Trojan Panel 前端"
    exit 0
  fi
  ...
  if [[ -z "${trojan_panel_ui_current_version}" || ! "${trojan_panel_ui_current_version}" =~ ^v.* ]]; then
    echo_content red "---> 当前版本不支持在线更新"
    exit 0
  fi

  echo_content yellow "提示：Trojan Panel 前端（trojan-panel-ui）当前版本为 ${trojan_panel_ui_current_version}，最新版本为 ${trojan_panel_ui_latest_version}"

  if [[ "${trojan_panel_ui_current_version}" != "${trojan_panel_ui_latest_version}" ]]; then
    echo_content green "---> 更新 Trojan Panel 前端"
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
      echo_content skyBlue "---> Trojan Panel 前端更新完成"
    else
      echo_content red "---> Trojan Panel 前端更新失败或运行异常，请尝试修复或卸载并重新安装"
    fi
  else
    echo_content skyBlue "---> 您已安装 Trojan Panel 前端的最新版本"
  fi
}
...
update_trojan_panel() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    echo_content red "---> 请先安装 Trojan Panel 后端"
    exit 0
  fi
  ...
  echo_content yellow "提示：Trojan Panel 后端（trojan-panel）当前版本为 ${trojan_panel_current_version}，最新版本为 ${trojan_panel_latest_version}"

  if [[ "${trojan_panel_current_version}" != "${trojan_panel_latest_version}" ]]; then
    echo_content green "---> 更新 Trojan Panel 后端"
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel$" -f "status=running") ]]; then
      echo_content skyBlue "---> Trojan Panel 后端更新完成"
    else
      echo_content red "---> Trojan Panel 后端更新失败或运行异常，请尝试修复或卸载并重新安装"
    fi
  else
    echo_content skyBlue "---> 您已安装 Trojan Panel 后端的最新版本"
  fi
}
...
update_trojan_panel_core() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-core$") ]]; then
    echo_content red "---> 请先安装 Trojan Panel 核心"
    exit 0
  fi
  ...
  echo_content yellow "提示：Trojan Panel 核心（trojan-panel-core）当前版本为 ${trojan_panel_core_current_version}，最新版本为 ${trojan_panel_core_latest_version}"

  if [[ "${trojan_panel_core_current_version}" != "${trojan_panel_core_latest_version}" ]]; then
    echo_content green "---> 更新 Trojan Panel 核心"
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-core$" -f "status=running") ]]; then
      echo_content skyBlue "---> Trojan Panel 核心 更新完成"
    else
      echo_content red "---> Trojan Panel 核心 更新失败或运行异常，请尝试修复或卸载并重新安装"
    fi
  else
    echo_content skyBlue "---> 您已安装 Trojan Panel 核心 的最新版本"
  fi
}
...
uninstall_caddy2() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-caddy$") ]]; then
    echo_content green "---> 卸载 Caddy2+https"

    docker rm -f trojan-panel-caddy &&
      rm -rf ${CADDY_DATA}

    echo_content skyBlue "---> Caddy2+https 卸载完成"
  else
    echo_content red "---> 请先安装 Caddy2+https"
  fi
}
...
uninstall_nginx() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-nginx") ]]; then
    echo_content green "---> 卸载 Nginx"

    docker rm -f trojan-panel-nginx &&
      rm -rf ${NGINX_DATA}

    echo_content skyBlue "---> Nginx 卸载完成"
  else
    echo_content red "---> 请先安装 Nginx"
  fi
}
...
uninstall_mariadb() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-mariadb$") ]]; then
    echo_content green "---> 卸载 MariaDB"

    docker rm -f trojan-panel-mariadb &&
      rm -rf ${MARIA_DATA}

    echo_content skyBlue "---> MariaDB 卸载完成"
  else
    echo_content red "---> 请先安装 MariaDB"
  fi
}
...
uninstall_redis() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-redis$") ]]; then
    echo_content green "---> 卸载 Redis"

    docker rm -f trojan-panel-redis &&
      rm -rf ${REDIS_DATA}

    echo_content skyBlue "---> Redis 卸载完成"
  else
    echo_content red "---> 请先安装 Redis"
  fi
}
...
uninstall_trojan_panel_ui() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-ui$") ]]; then
    echo_content green "---> 卸载 Trojan Panel 前端"

    docker rm -f trojan-panel-ui &&
      docker rmi -f jonssonyan/trojan-panel-ui &&
      rm -rf ${TROJAN_PANEL_UI_DATA}

    echo_content skyBlue "---> Trojan Panel 前端卸载完成"
  else
    echo_content red "---> 请先安装 Trojan Panel 前端"
  fi
}
...
uninstall_trojan_panel() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    echo_content green "---> 卸载 Trojan Panel 后端"

    docker rm -f trojan-panel &&
      docker rmi -f jonssonyan/trojan-panel &&
      rm -rf ${TROJAN_PANEL_DATA}

    echo_content skyBlue "---> Trojan Panel 后端卸载完成"
  else
    echo_content red "---> 请先安装 Trojan Panel 后端"
  fi
}
...
uninstall_trojan_panel_core() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-core$") ]]; then
    echo_content green "---> 卸载 Trojan Panel 核心"

    docker rm -f trojan-panel-core &&
      docker rmi -f jonssonyan/trojan-panel-core &&
      rm -rf ${TROJAN_PANEL_CORE_DATA}

    echo_content skyBlue "---> Trojan Panel 核心 卸载完成"
  else
    echo_content red "---> 请先安装 Trojan Panel 核心"
  fi
}

# 卸载所有 Trojan Panel 相关容器
uninstall_all() {
  echo_content green "---> 卸载所有 Trojan Panel 相关容器"

  docker rm -f $(docker ps -a -q -f "name=^trojan-panel")
  docker rmi -f $(docker images | grep "^jonssonyan/trojan-panel" | awk '{print $3}')
  rm -rf ${TP_DATA}

  echo_content skyBlue "---> 卸载所有 Trojan Panel 相关容器完成"
}

# 修改 Trojan Panel 前端端口
update_trojan_panel_ui_port() {
  if [[ -n $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
    echo_content green "---> 修改 Trojan Panel 前端端口"
    ...
    if [[ -z "${trojan_panel_ui_port}" ]]; then
      echo_content red "---> 未查询到 Trojan Panel 前端口"
      exit 0
    fi
    echo_content yellow "提示：Trojan Panel 前端（trojan-panel-ui）当前端口为 ${trojan_panel_ui_port}"

    read -r -p "请输入 Trojan Panel 前端的新端口（默认：8888）: " trojan_panel_ui_port
    [[ -z "${trojan_panel_ui_port}" ]] && trojan_panel_ui_port="8888"
    ...
    if [[ "$?" == "0" ]]; then
      echo_content skyBlue "---> Trojan Panel 前端端口修改完成"
    else
      echo_content red "---> Trojan Panel 前端端口修改失败"
    fi
  else
    echo_content red "---> Trojan Panel 前端未安装或运行异常，请修复或卸载并重新安装后重试"
  fi
}
...
redis_flush_all() {
  ...
  if [[ -z $(docker ps -q -f "name=^trojan-panel-redis$" -f "status=running") ]]; then
    echo_content red "---> Redis 运行异常"
    exit 0
  fi

  echo_content green "---> 刷新 Redis 缓存"

  read -r -p "请输入 Redis 的 IP 地址（默认：本地）: " redis_host
  [[ -z "${redis_host}" ]] && redis_host="127.0.0.1"
  read -r -p "请输入 Redis 的端口（默认：6378）: " redis_port
  [[ -z "${redis_port}" ]] && redis_port=6378
  while read -r -p "请输入 Redis 密码（必要）: " redis_pass; do
    if [[ -z "${redis_pass}" ]]; then
      echo_content red "密码不能为空"
    else
      break
  ...
  echo_content skyBlue "---> Redis 缓存刷新完成"
}
...
forget_pass() {
  while :; do
    echo_content yellow "1. 查询 MariaDB 密码"
    echo_content yellow "2. 查询 Redis 密码"
    echo_content yellow "3. 重置管理面板系统管理员的用户名和密码"
    ...
    1)
      if [[ -n $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
        ...
        echo_content red "---> 请在 Trojan Panel 后端服务器上执行"
      fi
      ;;
    2)
      if [[ -n $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    ...
    3)
      if [[ -n $(docker ps -a -q -f "name=^trojan-panel-mariadb$") ]]; then
        read -r -p "请输入 MariaDB 的 IP 地址（默认：本地）: " mariadb_ip
        [[ -z "${mariadb_ip}" ]] && mariadb_ip="127.0.0.1"
        read -r -p "请输入 MariaDB 的端口（默认：9507）: " mariadb_port
        [[ -z "${mariadb_port}" ]] && mariadb_port=9507
        read -r -p "请输入 MariaDB 的用户名（默认：root）: " mariadb_user
        ...
        if [[ "$?" == "0" ]]; then
          echo_content red "\n=============================================================="
          echo_content yellow "系统管理员默认用户名: sysadmin 默认密码: 123456"
          echo_content_yellow "请及时登录管理面板更改密码"
          echo_content_red "\n=============================================================="
        else
          echo_content_red "管理面板 sysadmin 用户名和密码重置失败"
        fi
      else
        echo_content_red "---> 请在 MariaDB 服务器上执行"
      fi
      ;;
    4)
      break
...
# 故障检测
failure_testing() {
  echo_content green "---> 开始故障排查"
  if [[ ! $(docker -v 2>/dev/null) ]]; then
    echo_content red "---> Docker 运行异常"
  else
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-caddy$") ]]; then
      if [[ -z $(docker ps -q -f "name=^trojan-panel-caddy$" -f "status=running") ]]; then
        echo_content red "---> Caddy2 运行异常，运行日志如下:"
        docker logs trojan-panel-caddy
      fi
      domain=$(cat "${DOMAIN_FILE}")
      if [[ -n ${domain} && ! -f "${CERT_PATH}${domain}.crt" ]]; then
        echo_content red "---> 证书申请异常，请尝试 1. 更改子域名重新构建 2. 重新启动服务器以重新申请证书 3. 重新构建并选择自定义证书选项"
        if [[ -f ${CADDY_LOG}error.log ]]; then
          echo_content red "Caddy2 错误日志如下:"
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-mariadb$") && -z $(docker ps -q -f "name=^trojan-panel-mariadb$" -f "status=running") ]]; then
      echo_content red "---> MariaDB 运行异常，运行日志如下:"
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-redis$") && -z $(docker ps -q -f "name=^trojan-panel-redis$" -f "status=running") ]]; then
      echo_content red "---> Redis 运行异常，运行日志如下:"
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel$") && -z $(docker ps -q -f "name=^trojan-panel$" -f "status=running") ]]; then
      echo_content red "---> Trojan Panel 后端运行异常，运行日志如下:"
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-ui$") && -z $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
      echo_content red "---> Trojan Panel 前端运行异常，运行日志如下:"
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-core$") && -z $(docker ps -q -f "name=^trojan-panel-core$" -f "status=running") ]]; then
      echo_content red "---> Trojan Panel 核心 运行异常，运行日志如下:"
  ...
  echo_content green "---> 故障排查结束"
}

log_query() {
  while :; do
    echo_content skyBlue "可以查询日志的应用程序如下:"
    echo_content yellow "1. Trojan Panel 后端"
    echo_content yellow "2. Trojan Panel 前端"
    echo_content yellow "3. 退出"
    read -r -p "请选择应用程序（默认: 3）: " select_log_query_type
    ...
    *)
    ...
    read -r -p "请输入要查询的行数（默认: 20）: " select_log_query_line_type
...
version_query() {
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-ui$") && -n $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
    trojan_panel_ui_current_version=$(docker exec trojan-panel-ui cat ${TROJAN_PANEL_UI_DATA}version)
    echo_content yellow "Trojan Panel Frontend(trojan-panel-ui) 的当前版本是 ${trojan_panel_ui_current_version}，最新版本是 ${trojan_panel_ui_latest_version}"
  fi
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel$") && -n $(docker ps -q -f "name=^trojan-panel$" -f "status=running") ]]; then
    trojan_panel_current_version=$(docker exec trojan-panel ./trojan-panel -version)
    echo_content yellow "Trojan Panel Backend(trojan-panel) 的当前版本是 ${trojan_panel_current_version}，最新版本是 ${trojan_panel_latest_version}"
  fi
  if [[ -n $(docker ps -a -q -f "name=^trojan-panel-core$") && -n $(docker ps -q -f "name=^trojan-panel-core$" -f "status=running") ]]; then
    trojan_panel_core_current_version=$(docker exec trojan-panel-core ./trojan-panel-core -version)
    echo_content yellow "Trojan Panel 核心(trojan-panel-core) 的当前版本是 ${trojan_panel_core_current_version}，最新版本是 ${trojan_panel_core_latest_version}"
  fi
}
...
main() {
  ...
  clear
  echo_content red "\n=============================================================="
  echo_content skyBlue "系统要求: CentOS 7+/Ubuntu 18+/Debian 10+"
  echo_content skyBlue "版本: v2.3.0"
  echo_content skyBlue "描述:一键安装Trojan Panel服务器"
  echo_content skyBlue "作者: jonssonyan <https://jonssonyan.com>"
  echo_content skyBlue "Github: https://github.com/trojanpanel"
  echo_content skyBlue "文档: https://trojanpanel.github.io"
  echo_content red "\n=============================================================="
  echo_content yellow "1. 安装 Trojan Panel 前端"
  echo_content yellow "2. 安装 Trojan Panel 后端"
  echo_content yellow "3. 安装 Trojan Panel 核心"
  echo_content yellow "4. 安装 Caddy2+https"
  echo_content yellow "5. 安装 Nginx"
  echo_content yellow "6. 安装 MariaDB"
  echo_content yellow "7. 安装 Redis"
  echo_content green "\n=============================================================="
  echo_content yellow "8. 更新 Trojan Panel 前端"
  echo_content yellow "9. 更新 Trojan Panel 后端"
  echo_content yellow "10. 更新 Trojan Panel 核心"
  echo_content green "\n=============================================================="
  echo_content yellow "11. 卸载 Trojan Panel 前端"
  echo_content yellow "12. 卸载 Trojan Panel 后端"
  echo_content yellow "13. 卸载 Trojan Panel 核心"
  echo_content yellow "14. 卸载 Caddy2+https"
  echo_content yellow "15. 卸载 Nginx"
  echo_content yellow "16. 卸载 MariaDB"
  echo_content yellow "17. 卸载 Redis"
  echo_content yellow "18. 卸载 所有与 Trojan Panel 相关的 docker"
  echo_content green "\n=============================================================="
  echo_content yellow "19. 修改 Trojan Panel 前端端口"
  echo_content yellow "20. 刷新 Redis 缓存"
  echo_content yellow "21. 更新证书"
  echo_content yellow "22. 忘记管理员密码"
  echo_content green "\n=============================================================="
